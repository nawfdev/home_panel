...
npm start
# or
cd go-backend && go run ./cmd/homepanel
```

Defaults: config `<root>/config/config.json`, data `<root>/data/db.json`, static `<root>/frontend`.
When run from `go-backend/`, path resolution automatically uses the parent repo root.
...
- Running from `go-backend/` now auto-detects the parent repo root, so `HOMEPANEL_ROOT` is optional.
