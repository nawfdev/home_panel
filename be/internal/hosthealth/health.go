...
const remoteCommand = `LC_ALL=C; cpu=$(top -bn1 2>/dev/null | awk '/Cpu\(s\)|%Cpu/{for(i=1;i<=NF;i++) if($i ~ /id,?$/){gsub(/,/,"",$(i-1)); print 100-$(i-1); exit}}'); mem=$(awk '/MemTotal/{t=$2}/MemAvailable/{a=$2}END{if(t>0) printf "%.2f",(t-a)*100/t}' /proc/meminfo); disk=$(df -P / 2>/dev/null | awk 'NR==2{gsub(/%/,"",$5);print $5}'); up=$(cut -d. -f1 /proc/uptime); net=$(awk -F: 'NR>2 {iface=$1; gsub(/^[ \t]+|[ \t]+$/,"",iface); if(iface!="lo"){split($2,a," "); rx+=a[1]; tx+=a[9]}} END{printf "%.0f %.0f",rx,tx}' /proc/net/dev); set -- $net; temp=""; for f in /sys/class/thermal/thermal_zone*/temp /sys/class/hwmon/hwmon*/temp*_input; do if [ -r "$f" ]; then v=$(cat "$f"); [ "$v" -gt 0 ] 2>/dev/null && { if [ "$v" -gt 1000 ] 2>/dev/null; then temp=$(awk "BEGIN{printf \"%.1f\",$v/1000}"); else temp=$(awk "BEGIN{printf \"%.1f\",$v}"); fi; break; }; fi; done; printf 'cpu=%s\nmem=%s\ndisk=%s\nuptime=%s\ndownloaded=%s\nuploaded=%s\ntemp=%s\n' "$cpu" "$mem" "$disk" "$up" "$1" "$2" "$temp"`
